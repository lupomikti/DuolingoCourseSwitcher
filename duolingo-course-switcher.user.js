...
var _ = unsafeWindow._; // Needed for old site, but not for new site
...
if (document.getElementById("app") === null) {
    ...
    document.head.appendChild($('<style type="text/css">'+
        '.language-sub-courses {position:absolute; top:-28px !important; color:#000; background-color: #fff; min-width: 150px; min-height: 50px; max-height: 500px; overflow-y: auto; display: none !important;}'+
        'html[dir="ltr"] .language-sub-courses {left:200px !important;}'+
        'html[dir="rtl"] .language-sub-courses {right:200px !important;}'+
        '</style>').get(0));
}
...
$(document).on({
    mouseenter: function() {
        ...
        // Get localized strings
        var languageNamesOld = duo.language_names_ui[A.ui_language];
        ...
        // Create top-level list using source languages
        $.each(courses, function( from, value ) {
            fromCourse = '<li class="language-choice choice"><a href="javascript:;"><span class="flag flag-svg-micro flag-'+from+'"></span><span>'+languageNamesOld[from]+'</span></a><ul class="dropdown-menu language-sub-courses '+from+'"><li class="head"><h6>'+header2+'</h6></li></ul></li>';
            ...
            $.each(value, function( fromx, v ) {
                to = v.language;
                sub = $('<li class="language-choice extra-choice" data-from="'+from+'" data-to="'+to+'"><a href="javascript:;"><span class="flag flag-svg-micro flag-'+to+'"></span><span>'+languageNamesOld[to]+'</span> <span class="gray">'+levelLabel+v.level+'</span></a></li>');
                sub.appendTo('ul.'+from);
